...
public class ThreadSafeBlockAccessor {
  ...
  public void init() {
    task = Bukkit.getScheduler().runTaskTimer(JourneySpigot.getInstance(), () -> {
      lock.lock();
      ...
      while (!requestQueue.isEmpty() && chunkMap.size() <= MAX_CACHED_CHUNKS) {
        ...
        World world = SpigotUtil.getWorld(req.data.domainId);
  ...
  public BlockData getBlock(Cell cell) {
    ...
    ChunkSnapshot snapshot = SpigotUtil.waitUntil(request.future);
