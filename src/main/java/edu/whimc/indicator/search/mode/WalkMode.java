...
public class WalkMode implements Mode<LocationLocatable, World> {
  @Override
      ...
      for (int offZ = -1; offZ <= 1; offZ++) {
        ...
        for (int offXFull = offX * offX; offXFull <= 1; offXFull++) {
          for (int offZFull = offZ * offZ; offZFull <= 1; offZFull++) {
            if (!origin.getBlockAtOffset(offXFull*offX, 0, offZFull*offZ).isPassable()) {
              break pathCheck;
            }
