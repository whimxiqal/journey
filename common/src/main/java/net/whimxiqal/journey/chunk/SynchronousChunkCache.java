 ...
 */
public class SynchronousChunkCache implements BlockProvider {
  ...
  public SynchronousChunkCache(FlagSet flagSet) {
    this.chunkCache = new ChunkCache(CentralChunkCache.MAX_CACHED_CHUNKS_PER_SEARCH);
  ...
  public void prepareChunks(Cell origin, Cell destination, int maxChunks) {
    ...
    int adjustedMaxChunks = Math.min(maxChunks, CentralChunkCache.MAX_CACHED_CHUNKS_PER_SEARCH / 2);
